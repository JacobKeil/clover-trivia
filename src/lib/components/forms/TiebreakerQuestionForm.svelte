...
>
	<div class="flex flex-col space-y-3">
		<div class="flex flex-col space-y-1">
			<label
				for="tb_prompt_{r_idx}"
				class="text-surface-content/80 text-xs font-semibold"
				class:text-rose-600={!question.question_text?.trim()}
			>
				Tiebreaker Question Prompt *
			</label>
			<input
				required
				id="tb_prompt_{r_idx}"
				type="text"
				placeholder="e.g. In miles, how long is the Amazon River?"
		...
		<div class="flex flex-col space-y-1">
			<label
				for="tb_ans_{r_idx}"
				class="text-surface-content/80 text-xs font-semibold"
				class:text-rose-600={missing_number(question.numeric_answer)}
			>
		...
		<div class="flex flex-col space-y-1">
			<label for="tb_notes_{r_idx}" class="text-surface-content/80 text-xs font-semibold">
				Host Notes / Clues
			</label>
			<input
				id="tb_notes_{r_idx}"
