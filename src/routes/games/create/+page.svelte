<script lang="ts">
	...
	const draft_key = 'clover-trivia:game-builder:create';
	...
	onMount(() => {
		...
		if (draft) {
			...
			if (draft.global_wagers) global_wagers = draft.global_wagers;
	...
	$effect(() => {
		if (!is_draft_ready) return;
		JSON.stringify({ form_state, global_wagers }, (key, value) =>
			key === 'image_file' ? undefined : value
		);
		const timeout = window.setTimeout(() => {
			save_game_builder_draft(draft_key, form_state, global_wagers, open_rounds);
